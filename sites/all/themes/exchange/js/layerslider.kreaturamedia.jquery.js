(function($) {
	...
	var layerSlider = function(el, options) {
		...
		ls.init = function(){
			...
			// NEW FEATURE v3.0 url- / deep linking layers

			if( document.location.hash ){
				ls.o.firstLayer = $(el).find('.ls-layer'+document.location.hash).index() + 1;
			}			

			// NEW FEATURE v3.0 added "normal" responsive mode with image and font resizing

			if( ls.o.width ){
				ls.g.sliderOriginalWidth = '' + ls.o.width;
			}else{
			...
			if( ls.o.yourLogo ){
				ls.g.yourLogo = $('<img>').appendTo($(el)).attr('style', ls.o.yourLogoStyle ).css({
					display: 'none'
				}).load(function(){
					...
					if( ls.g.yourLogo.css('left') != 'auto' ){
						ls.g.yourLogo.data( 'originalLeft', parseInt(ls.g.yourLogo.css('left')) );
					}else{
						ls.g.yourLogo.data( 'originalRight', parseInt(ls.g.yourLogo.css('right')) );					
					}
					if( ls.g.yourLogo.css('top') != 'auto' ){
						ls.g.yourLogo.data( 'originalTop', parseInt(ls.g.yourLogo.css('top')) );
					}else{
						ls.g.yourLogo.data( 'originalBottom', parseInt(ls.g.yourLogo.css('bottom')) );
					}
			...
			*/
			...
			// v3.0 adding 150ms delay because of styling bugs in some browsers
		...
		ls.timer = function(){
			...
			if( !ls.o.animateFirstLayer && !$(el).find('.ls-active').data('slidedelay') ){
				delaytime = $(el).find('.ls-layer:eq('+(ls.o.firstLayer-1)+')').data('slidedelay');
			}
		...
		ls.makeResponsive = function(layer, callback, bugfix){
			...
			for(var _sl=0;_sl < layer.children().length;_sl++){
				...
				if( !sl.hasClass('ls-bg') ){
					...
					}else if( ls.g.responsiveMode ){
						sl.css({
							left : parseInt(ol) * ls.g.ratio
						});
					}	
		...
		ls.resizeYourLogo = function(){
			...
			ls.g.yourLogo.css({
				...
				left : ls.g.yourLogo.data( 'originalLeft' ) * ls.g.ratio,
				right : ls.g.yourLogo.data( 'originalRight' ) * ls.g.ratio,
				top : ls.g.yourLogo.data( 'originalTop' ) * ls.g.ratio,
				bottom : ls.g.yourLogo.data( 'originalBottom' ) * ls.g.ratio
			}).fadeIn(300);
		};
		...
		ls.resizeSlider = function(){
			...
			// fullWidth mode (originally forceResponsive mode)
			...
			if( $(el).closest('.ls-wp-forceresponsive-container').length ){
				...
				$(el).closest('.ls-wp-forceresponsive-helper').css({
					width : $(window).width(),
					left : - $(el).closest('.ls-wp-forceresponsive-container').offset().left
				});
